...
const TestimonialCard = ({ testimonials }) => {
  ...
  const handlePrev = () => {
    ...
    setStartIndex((prevIndex) => prevIndex === 0 ? testimonials.length - testimonialsPerPage : prevIndex - 1);
    setActiveDot((prevDot) => prevDot === 0 ? testimonialsPerPage - 1 : prevDot - 1);
    startSlider();
  };

  const handleNext = () => {
    clearInterval(sliderInterval);
    setStartIndex((prevIndex) => prevIndex === testimonials.length - testimonialsPerPage ? 0 : Math.min(testimonials.length - testimonialsPerPage, prevIndex + 1));
    setActiveDot((prevDot) => prevDot === testimonialsPerPage - 1 ? 0 : prevDot + 1);
    startSlider();
  };
    ...
    <section className="text-gray-600 body-font">
      <div className="container px-5 py-20 mx-auto">
        ...
        <div className="flex justify-center items-center mb-4 mt-6">
          <p className='text-center text-[#488B80] text-4xl mt-2 item2'>Testimonials</p>
        </div>
        <div className="flex justify-center items-center mt-20">
          ...
          <button className="ml-4 text-2xl bg-[#54C8B4] text-white hover:bg-white item2 hover:text-[#54C8B4] py-2 px-6 rounded-tl-2xl rounded-br-2xl">
            Parents
          </button>
        </div>
        ...
        <div className="flex justify-center items-center mt-4">
          ...
          {[...Array(testimonialsPerPage)].map((_, index) => (
            <FaCircle key={index} className={`text-xs ${index === activeDot ? 'text-[#54C8B4] mr-4' : 'text-[#D9D9D9]'} mx-2 cursor-pointer`} onClick={() => setStartIndex(index * testimonialsPerPage)} />
          ))}
