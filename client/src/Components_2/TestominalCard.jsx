...
const TestimonialCard = ({ testimonials }) => {
  ...
  const mobile = isMobile() === false ? 3 : 1;

  const handlePrev = () => {
    clearInterval(sliderInterval);
    setStartIndex((prevIndex) =>
      prevIndex === 0 ? testimonials.length - mobile : prevIndex - 1
    );
  ...
  const handleNext = () => {
    clearInterval(sliderInterval);
    setStartIndex((prevIndex) =>
      prevIndex === testimonials.length - mobile
        ? 0
        : Math.min(testimonials.length - mobile, prevIndex + 1)
    );
    setActiveDot((prevDot) => (prevDot === 2 ? 0 : prevDot + 1));
    startSlider();
  };

  const visibleTestimonials = testimonials.slice(startIndex, startIndex + mobile);
    ...
    <section className="text-gray-600 body-font">
      <div className="container px-5 py-24 mx-auto">
        <div className="flex justify-center items-center ">
          <p className="text-center text-[#ED7E0A] text-2xl item5">
            - - - - Our love - - -
          </p>
        </div>
        <div className="flex justify-center items-center mb-4 mt-6 ">
          <p className="text-center text-[#488B80] text-4xl mt-2 item1">
            Faculty At Writo
          </p>
        </div>
              ...
              <div className="h-full text-center">
                ...
                <div className="flex justify-center mt-2">
                  ...
                  {[...Array(5 - testimonial.stars)].map((_, index) => (
                    <IoMdStarOutline
                      key={index}
                      className="text-[#E4E4E4] text-2xl"
                    />
                  ))}
                </div>
        ...
        <div className="flex justify-center items-center mt-4">
          <button
            onClick={handlePrev}
            className="mr-4 text-2xl text-gray-800 hover:text-[#54C8B4] font-semibold py-2 px-4 rounded-l"
          >
            <FaArrowLeft />
          </button>
          {[...Array(3)].map((_, index) => (
            <FaCircle
              key={index}
              className={`text-xs ${
                index === activeDot ? "text-[#54C8B4] mr-4" : "text-[#D9D9D9]"
              } mx-2 cursor-pointer`}
          ...
          <button
            onClick={handleNext}
            className="ml-4 text-2xl hover:text-[#54C8B4] text-gray-800 font-semibold py-2 px-4 rounded-r"
          >
            <FaArrowRight />
          </button>
        </div>
